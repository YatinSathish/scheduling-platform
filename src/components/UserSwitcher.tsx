...
export default function UserSwitcher({ role, variant = "pill" }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(endpoint)
      ...
      .then((data: User[]) => {
        setUsers(data);
        const active = new URLSearchParams(window.location.search).get("userId");
  ...
  return (
    <div ref={wrapperRef} className="relative">
      {variant === "heading" ? (
        ...
        >
          <span className="text-xl font-bold text-text-primary">Hi,</span>
          <span className="flex items-center gap-1 text-xl font-bold text-brand-primary px-1.5 py-0.5 rounded-lg group-hover:bg-brand-light transition-colors">
            {currentUser?.name ?? "…"}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="14"
                ...
                <li key={u.id}>
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 text-sm transition-colors hover:bg-surface-muted ${
                      active ? "text-brand-primary font-medium" : "text-text-primary"
                    }`}
                  >
                    {u.name}
                    {active && (
